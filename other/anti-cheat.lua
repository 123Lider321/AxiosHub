...
local Des = {}
if makefolder and not isfile("AxiosHub") then
    makefolder("AxiosHub")
end

G.Settings = {}
local Pcall = pcall(function()
    if isfile("AxiosHub//" .. Name) then
        readfile("AxiosHub//" .. Name)
    else
        writefile("AxiosHub//" .. Name, HttpService:JSONEncode(Des))
    end
end)

if isfile("AxiosHub//" .. Name) and readfile("AxiosHub//" .. Name) then
    Settings = HttpService:JSONDecode(readfile("AxiosHub//" .. Name))
end
...
G.Save = function()
    pcall(function()
        writefile("AxiosHub//" .. Name, HttpService:JSONEncode(Settings))
    end)
end
...
local Infinite = function(...) return wait(math.huge) end
for i,v in next, getgc() do
    if islclosure(v) then
        local Old = debug.getconstants(v)
        if table.find(Old, "Detected") or table.find(Old, "crash") or table.find(Old, "kick") then
            hookfunction(v, Infinite)
        end
    end
end
